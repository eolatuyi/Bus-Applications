...
src/app.cpp          application (poll, print, LCD text, LED bar)
        │
        ▼
MPU6050  PCF8591  HC595  LCD1602     device drivers (protocol + scaling)
        │            │         │
        ▼            ▼         ▼
...
| Device drivers | `src/mpu6050.*`, `src/pcf8591.*`, `src/hc595.*`, `src/lcd1602.*` | Device protocol, init, scaling to engineering units | Dashboard copy, polling policy, other devices' protocols |
